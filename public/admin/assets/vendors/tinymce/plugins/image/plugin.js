...
(function () {
      ...
      head(data.fileinput).fold(() => {
        ...
        blobToDataUri(file).then(dataUrl => {
          ...
          if (info.automaticUploads) {
            helpers.uploadFile(blobInfo).then(result => {
    ...
    const submitHandler = (editor, info, helpers) => api => {
      ...
      editor.editorUpload.uploadFilesAuto();
      api.close();
    };
    ...
    const uploadFile = editor => blobInfo => global$1(editor).upload([blobInfo], false).then(results => {
      if (results.length === 0) {
